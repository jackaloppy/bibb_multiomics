...
I built rat transcriptome reference using 10x tutorial, and used it to align c1-scrna sequence in cellranger count pipeline. See [QC report](https://jackaloppy.github.io/bibb_multiomics/qc_reports/c1_scrna_rat_web_summary.html). No low mapping rate alert. The median genes per cell is now ~1500. (Only warning _High Fraction of Reads Mapped Antisense to Genes_ is fine because as it says it's normal in single nuclei sequencing).
